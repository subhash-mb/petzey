...
export class DepartmentsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.departmentService.getAllDepartments().subscribe((res)=>{
     ...
     this.departmentService.getTotalNumberOfDepartments().subscribe(result=>{
        this.totalDepartments=result;
  ...
  getVetsByDepartmentId(departmentId:number){
    ...
    this.vetsList = [];
    this.departmentService.getListOfVets(departmentId).subscribe(result=>{
  ...
  getDepartmentById(){
    ...
    this.departmentService.getDepartmentById(this.departmentId).subscribe(data=>{
      ...
      this.loginForm.controls["departmentName"].setValue(this.previousData.departmentName);
      this.loginForm.controls["departmentEmail"].setValue(this.previousData.departmentEmail);
      this.loginForm.controls["status"].setValue(this.previousData.status);
      this.loginForm.controls["description"].setValue(this.previousData.description);
  ...
  editDepartment(){
    ...
    this.departmentToUpadte.departmentId=this.departmentId;
    this.departmentToUpadte.departmentEmail=this.loginForm.value.departmentEmail;
    ...
    this.departmentToUpadte.departmentName=this.loginForm.value.departmentName;
    console.log(this.departmentToUpadte);
    this.departmentService.editDepartmentById(this.departmentToUpadte).subscribe(
      (resp) => {
        console.log(resp);
...
deletedDepartment(){
  ...
  this.message=this.departmentService.deleteDepartmentById(this.departmentId).subscribe((delMessage)=>this.message=delMessage)
...
addDepartmentToVet(){
  ...
  for(let i=0;i<this.departmentList.length;i++){
    if(this.departmentList[i].departmentId==this.vetToUpdate.departmentId){
      flag=1;
      this.departmentService.addDepartmentToVet(this.vetToUpdate.vetnpiNo,this.vetToUpdate.departmentId).subscribe(
        (resp) => {
          console.log(resp);
